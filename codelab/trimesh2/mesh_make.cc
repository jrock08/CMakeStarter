...
#include "third_party/trimesh2/include/TriMesh.h"
#include "third_party/trimesh2/include/TriMesh_algo.h"
#include "third_party/trimesh2/include/XForm.h"
#include "third_party/trimesh2/include/noise3d.h"
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <iostream>
...
static inline void tess5(TriMesh *mesh, int v1, int v2, int v3, int v4,
                         int v5) {
  point c =
      0.2f * (mesh->vertices[v1] + mesh->vertices[v2] + mesh->vertices[v3] +
...
TriMesh *make_disc(int tess_th, int tess_r) {
  ...
  for (int j = 1; j < tess_r; j++) {
    ...
    for (int i = 0; i < tess_th; i++) {
      int i1 = (i + 1) % tess_th;
      mkquad(mesh, base + tess_th + i, base + tess_th + i1, base + i,
             base + i1);
    }
  }
...
TriMesh *make_cyl(int tess_th, int tess_h, float r = 1.0f) {
  ...
  for (int j = 0; j < tess_h; j++) {
    ...
    for (int i = 0; i < tess_th; i++) {
      int i1 = (i + 1) % tess_th;
      mkquad(mesh, base + i, base + i1, base + tess_th + i,
             base + tess_th + i1);
    }
  }
...
TriMesh *make_ccyl(int tess_th, int tess_h, float r = 1.0f) {
  ...
  for (int j = 1; j < tess_h; j++) {
    ...
    for (int i = 0; i < tess_th; i++) {
      int i1 = (i + 1) % tess_th;
      mkquad(mesh, base + tess_th + i1, base + tess_th + i, base + i1,
             base + i);
    }
  }
  ...
  for (int j = 0; j < tess_th; j++) {
    ...
    for (int i = 0; i < tess_ph; i++) {
      ...
      mesh->vertices[i + j * tess_ph] =
          circlepos + cos(ph) * r * circlepos + sin(ph) * r * vec(0, 0, -1);
    }
  }
  return mesh;
}
...
TriMesh *make_knot(int tess_th, int tess_ph, float r = 0.2f) {
  ...
  for (int j = 0; j < tess_th; j++) {
    float th = M_TWOPIf * j / tess_th;
    vec pos(2.0f * sin(2.0f * th) + cos(th), 2.0f * cos(2.0f * th) + sin(th),
            cos(3.0f * th));
    pos /= 3.0f;
    vec vel(4.0f * cos(2.0f * th) - sin(th), -4.0f * sin(2.0f * th) + cos(th),
            -3.0f * sin(3.0f * th));
    normalize(vel);
    for (int i = 0; i < tess_ph; i++) {
      float ph = M_TWOPIf * i / tess_ph;
      ...
      normalize(u);
      vec v = u CROSS vel;
      mesh->vertices[i + j * tess_ph] = pos + cos(ph) * r * v + sin(ph) * r * u;
    }
  }
  return mesh;
}
...
TriMesh *make_klein(int tess_th, int tess_ph) {
  ...
  for (int j = 0; j < tess_th; j++) {
    ...
    for (int i = 0; i < tess_ph; i++) {
      ...
      if (part == 0)
        mesh->vertices[i + j * tess_ph] =
            point((2.5f + 1.5f * cos(v)) * flipx * cos(u),
                  (2.5f + 1.5f * cos(v)) * sin(u), -2.5f * sin(v));
      else if (part == 1)
        mesh->vertices[i + j * tess_ph] =
            point((2.5f + 1.5f * cos(v)) * flipx * cos(u),
                  (2.5f + 1.5f * cos(v)) * sin(u), 3.0f * v);
      else if (part == 2)
        mesh->vertices[i + j * tess_ph] =
            point(2.0f + (2.0f - flipx * cos(u)) * cos(v), sin(u),
                  3.0f * M_PIf + (2.0f - flipx * cos(u)) * sin(v));
      else
        mesh->vertices[i + j * tess_ph] = point(
            2.0f + 2.0f * cos(v) - flipx * cos(u), sin(u), 3.0f * (M_PIf - v));
    }
  }
  return mesh;
}
...
TriMesh *make_helix(int tess_th, int tess_ph, float turns, float r = 0.25f) {
  ...
  for (int j = 0; j <= tess_th; j++) {
    ...
    for (int i = 0; i < tess_ph; i++) {
      float ph = M_TWOPIf * i / tess_ph;
      mesh->vertices[i + j * tess_ph] =
          helixpos + r * cos(ph) * xdir + r * sin(ph) * ydir;
    }
  }
  return mesh;
}
...
TriMesh *make_sphere_polar(int tess_ph, int tess_th) {
  ...
  for (int j = 0; j < tess_th - 2; j++) {
    ...
    for (int i = 0; i < tess_ph; i++) {
      int i1 = (i + 1) % tess_ph;
      mkquad(mesh, base + i, base + i1, base + tess_ph + i,
             base + tess_ph + i1);
    }
  }
...
TriMesh *make_platonic(int nfaces) {
  ...
  case 4:
    ...
    mkpoint(mesh, sqrt(8.0f) - sqrt(18.0f), sqrt(6.0f), -1.0f);
    mkpoint(mesh, sqrt(8.0f) - sqrt(18.0f), -sqrt(6.0f), -1.0f);
    for (int i = 0; i < 4; i++)
...
void usage(const char *myname) {
  ...
  fprintf(stderr,
          "	bump n sigma	n x n tessellated Gaussian bump of width sigma\n");
  fprintf(stderr,
          "	wave n omega	n x n tessellated sine wave of frequency omega\n");
  fprintf(stderr, "	frac n		n x n fractal landscape\n");
  fprintf(stderr, "	cube n		n x n tessellated cube\n");
  fprintf(stderr,
          "	disc n m	Circular disc, tessellated with m rings of n points\n");
  ...
  fprintf(stderr, "	sor n curve.txt	Surface of revolution: n copies of curve, "
                  "rot z axis\n");
  exit(1);
}
...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(argv[1], "plane") && argc > 4) {
    ...
    mesh =
        make_helix(atoi(argv[2]), atoi(argv[3]), ATOF(argv[4]), ATOF(argv[5]));
    outfilename = argv[6];
